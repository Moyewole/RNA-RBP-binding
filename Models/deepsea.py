...
class DeeperDeepSEA(nn.Module):
    ...
    def __init__(self, sequence_length, n_targets):
        ...
        self.conv_net = nn.Sequential(
            nn.Conv1d(4, 160, kernel_size=conv_kernel_size),
            nn.ReLU(inplace=True),
            nn.Conv1d(160, 320, kernel_size=conv_kernel_size),
            nn.ReLU(inplace=True),
            nn.MaxPool1d(
        ...
        self._n_channels = int(
            np.floor(
                (np.floor(
                 ...
                 - (reduce_by/2)) / pool_kernel_size))
        self.classifier = nn.Sequential(
            nn.Linear(480 * self._n_channels, 50*n_targets),
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(50*n_targets),
            nn.Linear(50*n_targets, n_targets),
            nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        """
        out = self.conv_net(x)
        reshape_out = out.view(out.size(0), 480 * self._n_channels)
